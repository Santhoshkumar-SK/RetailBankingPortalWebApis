...
namespace RulesMicroService.Services
{
    public class MonthlyjobService : IMonthlyjobService
    {
        ...
        /// <summary>
        /// It is the main function that scheduled to run every month for debiting service charges
        /// </summary>
        /// <returns></returns>
        public async Task toRunMonthlyjob()
        {
            ...
            {
                if (account.AccountBalance < 5000 && account.AccountBalance > 0)
                {
                    ...
                    TransactionDTO withDraw = new TransactionDTO() { AccountId = account.AccountID, Amount = (double)ServiceCharge };
                    TransactionStatusForDepositAndWithDraw status = await ApplyServiceCharge(withDraw);
                    if (status.Message == "Withdraw Successfull")
                    {
        ...
        public List<Account> getAccounts()
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                _log4net.Info("Getted All Accounts");
            }
